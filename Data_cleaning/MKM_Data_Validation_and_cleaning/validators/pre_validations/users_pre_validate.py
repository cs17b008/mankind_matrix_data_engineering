...
# )
# # optional schema validator (won't crash if YAML missing)
# try:
...
#         # 1) optional schema check
#         if HAS_SCHEMA_VALIDATOR:
#             try:
#                 validate_schema(df, TABLE)
#             except FileNotFoundError as _:
#                 print(f"[SCHEMA NOTICE] Expected schema YAML not found for {TABLE}; skipping schema check.")

#         # 2) basic data checks
#         issues = {
#             "not_null": check_not_null(df, ["id", "email"]),
#             "unique":   check_unique(df,   ["id", "email"]),
#         }
